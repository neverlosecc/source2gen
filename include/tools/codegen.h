...
namespace codegen {
	...
	public:
		...
		self_ref begin_block(const std::string& text, const std::string& access_modifier = "", bool increment_tabs_count = true, bool move_cursor_to_next_line = true) {
		...
		self_ref begin_function(const std::string& prefix, const std::string& type_name, const std::string& func_name, bool increment_tabs_count = true, bool move_cursor_to_next_line = true) {
		...
		self_ref static_field_getter(const std::string& type_name, const std::string& prop_name, const std::string& mod_name, const std::string& decl_class, const std::size_t index) {
	...
	private:
		...
		std::string escape_name(const std::string& name) {
			...
			for (std::size_t i = 0; i < name.size(); i++)
				result[i] = std::find(kBlacklistedCharacters.begin(), kBlacklistedCharacters.end(), name[i]) == std::end(kBlacklistedCharacters) ? name[i] : '_';
