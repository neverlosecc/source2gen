...
public:
    ...
    void start(const std::string& title)
    {
        ...
        {
            if (!AllocConsole()) throw std::runtime_error("cannot initialise console object more than once!");

            this->h_console_window_ = GetConsoleWindow();
            if (this->h_console_window_ == nullptr) throw std::runtime_error("cannot get console window!");
        }
        ...
        if (!status) { throw std::runtime_error("cannot set std output handle!"); }

        status = SetStdHandle(STD_ERROR_HANDLE, h_con_out);
        if (!status) { throw std::runtime_error("cannot set std error handle!"); }

        status = SetStdHandle(STD_INPUT_HANDLE, h_con_in);
        if (!status) { throw std::runtime_error("cannot set std input handle!"); }
    ...
    void update()
    {
        ...
        if (constexpr auto update_time = 150; duration.count() <= update_time) return;
    ...
    void stop(void)
    {
        ...
        if (const auto status = FreeConsole(); !status) { throw std::runtime_error("cannot free console!"); }

        if (old_out_) SetStdHandle(STD_OUTPUT_HANDLE, old_out_);
        if (old_err_) SetStdHandle(STD_ERROR_HANDLE, old_err_);
        if (old_in_) SetStdHandle(STD_INPUT_HANDLE, old_in_);

        if (const auto h_console_wnd = GetConsoleWindow(); h_console_wnd != nullptr)
            PostMessageA(
                         h_console_wnd, WM_CLOSE, 0, 0);

        if (this->m_active_) this->m_active_ = false;
    }
