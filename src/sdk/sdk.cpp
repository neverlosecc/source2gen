...
namespace sdk {
    ...
    namespace {
        ...
        [[nodiscard]]
        std::string StringReplace(std::string str, std::string_view search, std::string_view replace) {
        ...
        /// Decomposes a templated type into its components, keeping template syntax for later reassembly.
        ...
        // TOOD: move local functions into an anonymous namespace
        ...
        void AssembleClass(codegen::generator_t::self_ref builder, const CSchemaClassBinding& class_) {
            ...
            const std::string parent_class_name =
                (class_parent != nullptr) ? MaybeWithModuleName(*class_parent->m_pTypeScope, class_parent->m_pszName) : "";
            ...
            if (class_info.m_pFieldMetadataOverrides && class_info.m_pFieldMetadataOverrides->m_iTypeDescriptionCount > 1) {
                ...
                for (std::uint64_t s = 0; s < dm->m_iTypeDescriptionCount; s++) {
                    ...
                    if (const auto it = std::ranges::find_if(cached_fields,
                                                             [t, field_name](const auto& f) { return f.first == field_name && f.second == t->m_iOffset; });
    ...
    [[nodiscard]]
    std::set<TypeDependency> GetRequiredDependenciesForType(const CSchemaType& type) {
        ...
        if (type.m_pTypeScope != nullptr) {
            ...
            for (const auto& dirty_type_name : destructured) {
                ...
                if (auto module{GetModuleOfTypeInScope(*type.m_pTypeScope, type_name)}) {
                    const auto source =
                        (!is_used_in_non_odr_container && IsOdrUse(dirty_type_name)) ? TypeSource::include : TypeSource::forward_declaration;
    ...
    std::set<TypeDependency> FindRequiredDependenciesForClass(const CSchemaClassBinding& class_) {
        std::set<TypeDependency> result{};

        for (const auto& field : std::span{class_.m_pFields, static_cast<std::size_t>(class_.m_nFieldSize)}) {
            const auto includes = GetRequiredDependenciesForType(*field.m_pSchemaType);
        ...
        if (const auto* base_classes = class_.m_pBaseClasses; base_classes != nullptr) {
            ...
            const auto includes = GetRequiredDependenciesForType(*base_classes[0].m_pClass->m_pSchemaType);
            result.insert(includes.begin(), includes.end());
        }
    ...
    void GenerateClassSdk(std::string_view module_name, const CSchemaClassBinding& class_) {
        ...
        const auto dependencies = FindRequiredDependenciesForClass(class_);
